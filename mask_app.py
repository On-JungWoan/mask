...
import sys
sys.path = ['./segment_anything_2/'] + sys.path
...
class PromptGUI(object):
    ...
    def save_masks_to_dir(self, output_dir: str) -> str:
        ...
        img_output_dir = f"{output_dir}/img"
        npy_output_dir = f"{output_dir}/npy"
        ...
        for img_path, clr_mask, id_mask in zip(self.img_paths, self.color_masks_all, self.index_masks_all):
            ...
            out_path = f"{img_output_dir}/{name}"
            iio.imwrite(out_path, clr_mask)
            np_out_path = f"{npy_output_dir}/{name[:-4]}.npy"
...
def make_demo(
    checkpoint_dir, model_cfg, root_dir, vid_name, img_name, mask_name,
):
    ...
    with gr.Blocks() as demo:
        ...
        def update_mask_dir(root_dir, mask_name, seq_name):
            return os.path.abspath(f"{root_dir}/../{mask_name}/{seq_name}")
        ...
        mask_name_field.submit(
            update_mask_dir,
            [root_dir_field, mask_name_field, img_name_field],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", type=str, default="./inputs")
